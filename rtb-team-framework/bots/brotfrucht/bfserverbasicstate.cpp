/************************************************************************
    $Id: bfserverbasicstate.cpp,v 1.2 2005/02/24 10:27:53 jonico Exp $
...
namespace Brotfrucht {
	...
	void  BFServerBasicState::sendServerTips () { //throw (StrategyException,bad_exception) {
		...
		if (_newGame) {
			_newGame=false;
			Logger* logger=_bfsharedssp->getLogger();
			int number=_bfsharedssp->getRobotNumber();
			_numberconv << number;
			logger->logMessage(3,"We are "+_numberconv.str()+ " friendly robots. Our names: ");
			_numberconv.clear();
			_numberconv.str("");
			for (int i=0;i<number;++i)
				logger->logMessage(2,_bfsharedssp->getRobotName(i));
		}
	}
	...
	*/
	void BFServerBasicState::receiveRTBMessageCollision(object_type type,double angle) { //throw (StrategyException, bad_exception) {
		...
		_numberconv << "x: " << x << " y: " << y;
		_bfsharedssp->getLogger()->logMessage(3,_robotName+" was hit on position: "+_numberconv.str());
		_numberconv.clear();
		_numberconv.str("");
	}
